...
def run(args):
    ...
    if task_name:
        ...
        if (not input_points is None) and (not input_labels is None):
            ...
            if task_name in ["replace", "대체"]:

                text_prompt_label = "Please type a prompt to be replaced:" if args.lang == "en" else "대체할 업젝트 프롬프트를 입력해 주세요:"
                text_prompt = st.text_input(label = text_prompt_label)
            
            Action(im_path, ckpts_path = args.ckpt_path, input_points = input_points, lama_config = args.lama_config, lama_ckpt = args.lama_ckpt,
                    input_labels = input_labels, device = args.device, dks = args.dilate_kernel_size, lang = args.lang, text_prompt = text_prompt).run()
